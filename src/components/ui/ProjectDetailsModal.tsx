    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-4xl max-h-[92vh] overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl ring-1 ring-slate-200 md:p-8">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div>
            <span className="rounded-full bg-[#edfaff] px-3 py-1 text-xs font-bold uppercase tracking-wider text-[#1a6db2]">
              {project.category}
            </span>
            <h2 className="mt-2 text-2xl font-black tracking-tight text-[#123d66] md:text-3xl">
              {project.title}
            </h2>
          </div>
          <button
        ...
        <div className="mt-6 overflow-hidden rounded-2xl bg-slate-100">
          <div className="relative h-72 w-full md:h-96">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#0a2c4a]/70 via-transparent to-transparent" />
          ...
          <div className="flex gap-2 p-3 bg-slate-50 border-t border-slate-200/80 overflow-x-auto">
            <button
              ...
              className={`h-14 w-20 shrink-0 overflow-hidden rounded-lg border-2 transition ${
                displayImage === project.mainImage ? 'border-[#1a6db2] scale-105' : 'border-transparent opacity-70 hover:opacity-100'
              }`}
            >
            ...
            {project.galleryImages.map((img, i) => (
              <button
                ...
                className={`h-14 w-20 shrink-0 overflow-hidden rounded-lg border-2 transition ${
                  displayImage === img ? 'border-[#1a6db2] scale-105' : 'border-transparent opacity-70 hover:opacity-100'
                }`}
              >
        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-[1.5fr_1fr]">
              ...
              {activeTab === 'features' && (
                <div className="grid gap-3 sm:grid-cols-2">
                  {project.features.map((feature, idx) => (
                    ...
                    >
                      <CheckCircle2 className="h-4 w-4 text-[#1a6db2] shrink-0" />
                      <span>{feature}</span>
                    </div>
                  ))}
                </div>
          ...
          <div className="rounded-2xl bg-[#f7f9fc] p-6 ring-1 ring-slate-200">
            <h4 className="text-xs font-bold uppercase tracking-[0.2em] text-[#1a6db2]">
            ...
            <div className="mt-4 divide-y divide-slate-200/80 text-sm">
              <div className="flex items-center justify-between py-2.5">
                <span className="flex items-center gap-2 text-slate-500">
                  <MapPin className="h-4 w-4 text-[#123d66]" /> Location
                </span>
                <span className="font-semibold text-slate-800">{project.location.split(',')[0]}</span>
              </div>
              <div className="flex items-center justify-between py-2.5">
                <span className="flex items-center gap-2 text-slate-500">
                  <Maximize className="h-4 w-4 text-[#123d66]" /> Built Area
                </span>
                <span className="font-semibold text-slate-800">{project.area}</span>
              </div>
              <div className="flex items-center justify-between py-2.5">
                <span className="flex items-center gap-2 text-slate-500">
                  <Calendar className="h-4 w-4 text-[#123d66]" /> Year Completed
                </span>
                <span className="font-semibold text-slate-800">{project.year}</span>
              </div>
              <div className="flex items-center justify-between py-2.5">
                <span className="flex items-center gap-2 text-slate-500">
                  <Home className="h-4 w-4 text-[#123d66]" /> Classification
                </span>
            ...
            <button
              ...
              className="mt-6 flex w-full items-center justify-center gap-2 rounded-full bg-[#1a6db2] py-3 text-sm font-bold text-white shadow-md shadow-[#1a6db2]/20 transition hover:bg-[#145290]"
            >
