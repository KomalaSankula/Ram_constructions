          ...
          {categories.map((cat) => (
            <button
              ...
              className={`rounded-full px-6 py-2 text-sm font-bold transition-all duration-200 ${
                activeCategory === cat
                  ? 'bg-[#0d55c8] text-white shadow-md shadow-[rgb(26_109_178/0.5)] scale-105'
        ...
        <div className="mt-8 sm:mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filteredProjects.map((project) => (
            <div
              ...
              className="group relative cursor-pointer overflow-hidden rounded-[26px] bg-slate-50 shadow-card ring-1 ring-slate-200/80 transition-all duration-500 hover:-translate-y-2 hover:shadow-elevated hover:ring-[rgb(26_109_178/0.5)]"
            >
              <div className="relative h-64 sm:h-72 w-full overflow-hidden bg-slate-200">
                ...
                <div className="absolute top-4 right-4 flex h-9 w-9 items-center justify-center rounded-full bg-white/80 text-[#0d55c8] backdrop-blur-sm opacity-0 transition-all duration-300 group-hover:opacity-100 group-hover:scale-105 shadow-md">
                  <Eye className="h-4 w-4" />
                </div>

                <div className="absolute bottom-4 left-4 right-4 text-white">
                  <div className="flex items-center gap-1.5 text-xs text-[#39a1d6]">
                  ...
                  <h3 className="mt-1 text-xl font-bold tracking-tight text-white group-hover:text-[#39a1d6] transition-colors">
                    {project.title}
                  </h3>
                </div>
              </div>
